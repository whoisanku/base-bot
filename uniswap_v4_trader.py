...
# EntryPoint v0.6.0, as seen in the user's transaction
...
# ABI for the ERC-4337 EntryPoint contract
...
class UniswapV4Trader:
    def __init__(self):
        ...
        self.signer = Account.from_key(PRIVATE_KEY)
        self.entry_point = self.web3.eth.contract(
            address=ENTRY_POINT_ADDRESS, abi=ENTRY_POINT_ABI
        )
    ...
    def _user_op_to_tuple(self, user_op_dict):
        """Convert dict to the tuple struct expected by EntryPoint ABI."""
        return (
    ...
    def _send_to_bundler(self, signed_user_op: dict):
        ...
        if is_biconomy:
            # 💡 Biconomy expects the *envelope* object
